...
class Init {
	...
	private function load_hooks() {
		...
		add_action( 'wp_ajax_lifeawesome_set_admin_notice_viewed', array( $this, 'notice_closed' ) );
		...
		add_filter(
						...
						function () {
							return trailingslashit( get_template_directory_uri() ) . 'assets/img/logo-icon-lifeawesome.svg';
						}
					);
	}
	...
	public function setup_theme() {
		load_theme_textdomain( 'lifeawesome', get_template_directory() . '/languages' );
	}
	...
	public function change_stylesheet_directory() {
		return LIFEAWESOME_DIR . 'gutenverse-files/';
	}
	...
	public function notice_closed() {
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'lifeawesome_admin_notice' ) ) {
  ...
  array (
	...
	public function theme_config() {
		...
		$config = array(
			...
			'version'       => LIFEAWESOME_VERSION,
			'images'        => get_template_directory_uri() . '/assets/img/',
			'title'         => esc_html__( 'LifeAwesome', 'lifeawesome' ),
			'description'   => esc_html__( 'LifeAwesome is a Multipurpose Business WordPress Block Theme built for professionals and businesses that want to create a modern, high-performing website without writing a single line of code. Built with full site editing and powered by Gutenverse, this theme delivers a flexible and fully customizable platform with 50+ ready-to-import demo sites and 80+ block elements to help you launch your site in minutes. As the first Multipurpose Business theme fully compatible with FSE, LifeAwesome is ideal for digital agencies, finance consultants, startup companies, life coaches, human resource agencies, and adventure travel businesses looking to establish a strong online presence. With responsive layouts, a live drag-and-drop editor, global style manager, and well-structured block patterns, you can easily customize pages, services, portfolios, and testimonials to match your brand. Optimized for speed and performance, LifeAwesome allows you to build a professional website that reflects your identity and grows with your Multipurpose Business needs.', 'lifeawesome' ),
			'pluginTitle'   => esc_html__( 'Plugin Requirement', 'lifeawesome' ),
			'pluginDesc'    => esc_html__( 'This theme require some plugins. Please make sure all the plugin below are installed and activated.', 'lifeawesome' ),
			'note'          => '',
			'note2'         => '',
			'demo'          => '',
			...
			'doneText'      => esc_html__( 'Gutenverse Plugin Installed', 'lifeawesome' ),
  ...
  '1x' => 'https://ps.w.org/gutenverse-companion/assets/icon-128x128.png?rev=3162415',
		...
		if ( 'themes.php' === $pagenow && isset( $_GET['page'] ) && 'lifeawesome-dashboard' === sanitize_key( wp_unslash( $_GET['page'] ) ) ) {
			$admin_config = array(
